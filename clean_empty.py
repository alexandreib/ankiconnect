...
subprocess.run([sys.executable, "cleanup_tags.py", "--mode", "retranslate", "--scope", "empty"], check=True)

print("\n=== Updating tags (all cards) ===")
subprocess.run([sys.executable, "cleanup_tags.py", "--mode", "tags"], check=True)

print("\n=== Importing back to Anki ===")
subprocess.run([sys.executable, "anki_hsk.py", "import"], check=True)
